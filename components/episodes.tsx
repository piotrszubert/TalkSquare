    ...
    return (
        <div>
            {episodesToDisplay.map((episode, index) => (
                <div
                    key={index}
                    className={`-mx-5 hover:bg-white hover:text-black px-5 hover:cursor-pointer py-3 relative ${hoveredIndex === index ? 'hovered' : ''
                        }`}
                    onMouseEnter={() => setHoveredIndex(index)}
                    onMouseLeave={() => setHoveredIndex(-1)}
                >
                    <Link className="absolute inset-0" href={`/episodes/${episode.id}`}></Link>
